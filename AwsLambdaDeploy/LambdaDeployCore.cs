...
namespace AwsLambdaDeploy
{
    public class LambdaDeployCore
    {
            ...
            string scheduleName)
        {
            ...
            var rules = cloudwatchClient.ListRulesAsync(new ListRulesRequest {NamePrefix = scheduleName}).Result;
            if (rules.Rules.Any())
            {
                var targetsForRule = cloudwatchClient.ListTargetsByRuleAsync(new ListTargetsByRuleRequest { Rule = scheduleName }).Result.Targets;
                if (targetsForRule.Any())
                {
                    cloudwatchClient.RemoveTargetsAsync(new RemoveTargetsRequest { Ids = targetsForRule.Select(x => x.Id).ToList(), Rule = scheduleName }).Wait();
                }
            ...
            string scheduleRuleArn)
        {
            ...
            client.AddPermissionAsync(new AddPermissionRequest
            {
            ...
            CreateFunctionRequest createFunctionRequest)
        {
            ...
            {
                client.DeleteFunctionAsync(functionName).Wait();
            }
