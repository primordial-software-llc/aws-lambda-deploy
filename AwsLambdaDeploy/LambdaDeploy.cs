...
namespace AwsLambdaDeploy
{
    public class LambdaDeploy
    {
            ...
            int numberOfDeploymentsPerRegion)
        {
            Parallel.ForEach(regions, region =>
            {
                for (int ct = 0; ct < numberOfDeploymentsPerRegion; ct++)
                {
                    ...
                    {
                        client.DeleteFunctionAsync(GetFunctionName(functionName, ct)).Wait();
                    }
                }
                new LambdaDeployCore().DeleteFunctionSchedule(credentials, region, GetScheduleName(functionName));
            });
        }
            ...
            TimeSpan timeout)
        {
            ...
            var outputPath = LambdaDeployCore.GetOutputPath(projectPath);
            ...
            {
                var lambdaSymphony = new LambdaDeployCore();
                Parallel.ForEach(regions, region =>
                {
